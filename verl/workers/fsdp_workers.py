...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        elif rollout_name == 'sci':
            from verl.workers.rollout.vllm_rollout.sci_rollout import SciRollout
            from verl.workers.sharding_manager import FSDPVLLMShardingManager
            log_gpu_memory_usage(f'Before building {rollout_name} rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            rollout = SciRollout(model_path=local_path,
                                    config=self.config.rollout,
                                    tokenizer=self.tokenizer,
                                    model_hf_config=self.actor_model_config,
                                    server_url=self.config.rollout.url)
    ...
    def compute_multi_turn_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output = self.actor.compute_multi_turn_log_prob(data=data)
            output = DataProto.from_dict(tensors={'old_log_probs': output},
                                         meta_info={'temperature': self.config.rollout.temperature})
            output = self.ulysses_sharding_manager.postprocess_data(output)
    ...
    def compute_ref_multi_turn_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output = self.ref_policy.compute_multi_turn_log_prob(data=data)
            output = DataProto.from_dict(tensors={'ref_log_prob': output})
            output = self.ulysses_sharding_manager.postprocess_data(output)
