...
class SciRollout(BaseRollout):
    ...
    def fake_init(self, task, var, **kwargs):
        if self.debug:
            ...
            res = requests.post(self.server_url + "/reset", json={'task': task, 'var': var}) 
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        all_completion_ids = [None] * len(task)
        all_completion_mask = [None] * len(task)
        all_prompt_completion_mask = [None] * len(task)
        all_position_ids = [None] * len(task)
        ...
        with self.update_sampling_params(**kwargs):
            ...
            system_info, tasks_des = self.fake_init(task, var)
            ...
            while any([s['skip_flag'] != True for s in states]) and max_steps > 0:
                ...
                for i, state in enumerate(states):
                    ...
                    if states[i]["completed"] == False and batch_done[i] == True:
