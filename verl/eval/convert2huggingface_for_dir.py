...
def main():
    ...
    ckpt_dir = "/inspire/hdd/ws-8207e9e2-e733-4eec-a475-cfa1c36480ba/embodied-multimodality/qiuxipeng-24028/xpqiu/lji/verl_mod/verl/outputs_v2/20250404/alf_v2/20250404_1749/rank_0/ckpt"
    converted_ckpt_dir = "/inspire/hdd/ws-8207e9e2-e733-4eec-a475-cfa1c36480ba/embodied-multimodality/qiuxipeng-24028/xpqiu/lji/verl_mod/verl/outputs_v2/20250404/alf_v2/20250404_1749/rank_0/converted_ckpt"
    for ckpt_name in os.listdir(ckpt_dir):
        print(f"processing {ckpt_name}")
        if 'global_step' not in ckpt_name:
            continue
        
        # Loading FSDP checkpoint (optional: these three lines can be skipped. Prerequisite: actor_module must be preloaded)
        fsdp_checkpoint_path = f"{ckpt_dir}/{ckpt_name}/actor"
        huggingface_checkpoint_path = f"{fsdp_checkpoint_path}/huggingface"
        state_dict = load_sharded_model(fsdp_checkpoint_path)
        actor_module.load_state_dict(state_dict)

        actor_module.save_pretrained(huggingface_checkpoint_path)
        tokenizer.save_pretrained(huggingface_checkpoint_path)
