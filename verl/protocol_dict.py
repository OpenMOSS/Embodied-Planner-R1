...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(val, np.ndarray) and val.dtype == object, 'data in the non_tensor_batch must be a numpy.array with dtype=object'
    ...
    def from_dict(cls, tensors: Dict[str, torch.Tensor], non_tensors=None, meta_info=None, num_batch_dims=1):
        ...
        """
        assert len(tensors) > 0 or len(non_tensors) > 0, 'tensors or non_tensors must not be empty'
        ...
        for key, val in non_tensors.items():
            ...
            if batch_size is None:
                ...
                current_batch = val.shape[:num_batch_dims]
                assert batch_size == current_batch, \
                    f'Not all the val in non_tensors have the same batch size with batch_dims={num_batch_dims}. Got {pivot_key} has {batch_size}, {key} has {current_batch}'
        
        assert batch_size is not  None, "At least have one data!!"

        if tensors == {}:
            tensors['pad_input_ids'] = torch.tensor([[0] for _ in range(batch_size[0])])
    ...
    def pop(self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None) -> 'DataProto':
        ...
        """
        assert batch_keys is not None or non_tensor_batch_keys is not None
